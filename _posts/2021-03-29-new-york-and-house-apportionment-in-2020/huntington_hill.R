...
for (i in seq(1920, 2020, by=10)) {
      ...
      assign(str_c("hunt_hill_sequence_df_", year), hunt_hill_sequence_df)
      assign(str_c("five_winners_", year), five_winners)
      assign(str_c("five_losers_", year), five_losers)
}

hunt_hill_sequence_result <- bind_rows(hunt_hill_sequence_df_1920$a, 
...
model_hhill <- lm(seats_minus_quota ~ log(pop_pct), data = hunt_hill_sequence_result)
summary(model_hhill)
